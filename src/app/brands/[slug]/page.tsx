...
export default async function BrandPage({ params }: Props) {
    ...
    if (!brand) notFound();
    const products = await getProductsByTaxonomyCached('brand', brand, 200);
    const canonical = `/brands/${slugifySeoValue(brand)}`;
    ...
    return <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(graph) }} /><CatalogLandingPage eyebrow="Shop by manufacturer" title={`${brand} products`} summary={summary} products={products} guidance={guidance} faq={faq} relatedLinks={relatedLinks} filtersHref={`/products?brand=${encodeURIComponent(brand)}`} /></>;
}
