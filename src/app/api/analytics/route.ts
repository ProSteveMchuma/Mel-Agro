...
export async function GET(request: Request) {
    try {
        ...
        console.error('Analytics search lookup failed:', error);
        return NextResponse.json({ searches: [], message: 'Search analytics are temporarily unavailable' }, { status: 500 });
    }
}
...
export async function POST(request: Request) {
    try {
        ...
        if (event === 'search') {
            ...
            await recordPaidPurchase({ orderId, order: (order.data() || {}) as Record<string, unknown> });
        }

        return NextResponse.json({ success: true });
    } catch (error) {
        console.error('Analytics ingest failed:', error);
        return NextResponse.json({ success: false, message: 'Analytics event could not be stored' }, { status: 500 });
    }
}
