...
export async function POST(request: Request) {
  ...
  const update = input.action === "role" ? { role: input.role } : { status: input.status };
  const batch = adminDb.batch();
  batch.update(ref, { ...update, updatedAt: now });
  batch.set(auditRef, { action: input.action === "role" ? "user_role_changed" : "user_status_changed", actorId: actor.uid, actorEmail: actor.email || null, targetId: input.userId, before: input.action === "role" ? { role: snapshot.data()?.role } : { status: snapshot.data()?.status }, after: update, createdAt: now });
  await batch.commit();
