...
export async function GET(request: Request) {
  ...
  const [orders, products, users, payments] = await Promise.all([
    adminDb.collection("orders").orderBy("date", "desc").limit(300).get(),
    adminDb.collection("products").limit(600).get(),
    adminDb.collection("users").limit(500).get(),
    adminDb.collection("c2bPayments").orderBy("recordedAt", "desc").limit(200).get(),
  ]);
