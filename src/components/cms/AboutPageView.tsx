...
import type { AboutBlock, AboutBlocksPage } from "@/lib/cms-blocks";

export default function AboutPageView({ page }: { page: AboutBlocksPage }) {
  return (
    <main className="flex-grow">
      {page.blocks.map((block) => (
        <AboutBlockView key={block.id} block={block} />
      ))}
    </main>
  );
}
...
function AboutBlockView({ block }: { block: AboutBlock }) {
  switch (block.type) {
    ...
    case "who":
      return <WhoSection data={block.data} />;
    case "missionVision":
      return <MissionVisionSection data={block.data} />;
    case "values":
      return <ValuesSection data={block.data} />;
    case "cta":
...
function HeroSection({
  ...
  data: Extract<AboutBlock, { type: "hero" }>["data"];
}) {
  return (
      ...
      <div className="container-custom relative z-10">
        <div className="max-w-4xl">
          ...
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-black text-white mb-8 tracking-tighter leading-[0.95]">
            ...
            {data.heroTitleSuffix ? ` ${data.heroTitleSuffix}` : ""}
          </h1>
          <p className="text-xl md:text-2xl text-gray-400 font-medium leading-relaxed max-w-3xl">{data.heroSubtitle}</p>
        </div>
      </div>
    </section>
  );
...
function WhoSection({
  ...
  data: Extract<AboutBlock, { type: "who" }>["data"];
}) {
  return (
        ...
        <div className="bg-white/80 backdrop-blur-2xl rounded-[40px] shadow-2xl shadow-black/5 border border-white p-8 md:p-16 lg:p-20">
          <div className="flex flex-col lg:flex-row gap-12 items-center">
            <div className="lg:w-1/2">
              ...
              <h2 className="text-4xl md:text-5xl font-black text-gray-900 mb-8 tracking-tight">{data.whoTitle}</h2>
            ...
            <div className="lg:w-1/2 relative">
              <div className="aspect-square bg-gradient-to-br from-melagri-primary to-green-200 rounded-[3rem] overflow-hidden shadow-2xl transform lg:rotate-3 hover:rotate-0 transition-transform duration-700">
                ...
                <div className="p-12 h-full flex flex-col justify-end text-white">
                  ...
                  <p className="text-2xl font-bold leading-tight italic">{data.quote}</p>
                </div>
              </div>
            </div>
          </div>
...
function MissionVisionSection({
  ...
  data: Extract<AboutBlock, { type: "missionVision" }>["data"];
}) {
  return (
      ...
      <div className="container-custom">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="group relative p-10 bg-white rounded-[2.5rem] shadow-sm border border-gray-100 hover:shadow-xl hover:-translate-y-2 transition-all duration-500">
            ...
            <p className="text-lg text-gray-500 leading-relaxed font-medium">{data.mission}</p>
          </div>
          ...
          <div className="group relative p-10 bg-white rounded-[2.5rem] shadow-sm border border-gray-100 hover:shadow-xl hover:-translate-y-2 transition-all duration-500">
            ...
            <p className="text-lg text-gray-500 leading-relaxed font-medium">{data.vision}</p>
          </div>
        </div>
      </div>
    </section>
...
function ValuesSection({
  ...
  data: Extract<AboutBlock, { type: "values" }>["data"];
}) {
  return (
...
function CtaSection({
  ...
  data: Extract<AboutBlock, { type: "cta" }>["data"];
}) {
  return (
    <section className="py-24 bg-gray-900 relative overflow-hidden text-center">
      <div className="absolute inset-0 opacity-20 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')]"></div>
      <div className="container-custom relative z-10">
        <h2 className="text-5xl md:text-7xl font-black text-white mb-8 tracking-tighter">{data.ctaTitle}</h2>
        <p className="text-xl text-gray-400 mb-12 max-w-2xl mx-auto font-medium">{data.ctaBody}</p>
        <Link
          href={data.ctaHref}
